#!/bin/sh
# QNX 8.0 QEMU launcher (template).
#
# Edit the values marked CHANGE_ME before first use, then run from the
# directory that holds your OVMF_VARS.fd (the relative paths below resolve
# against the working directory, so launch from ~/qnx-qemu, not the repo).
#
# Tweakable settings (safe defaults shown):
#   -smp $(nproc)   number of vCPUs; defaults to all host cores. Lower it if
#                   you want to leave host headroom.
#   -m 16G          guest RAM. 8G works for most ports; large C++ builds
#                   (webkit, llvm) are happier with 16G or more.
#   hostfwd 2227    host port forwarded to guest SSH (22). Change only if 2227
#                   is taken on your host; if you do, update TARGET.md to match.
#   --enable-kvm    requires /dev/kvm on the host. Drop this line if KVM is
#                   unavailable (much slower, but works).
#
# Image and firmware paths (CHANGE_ME):
